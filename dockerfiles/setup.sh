...
sed -i 's/user = nobody/;user = nobody/1' /etc/php81/php-fpm.d/www.conf
sed -i 's/group = nobody/;group = nobody/1' /etc/php81/php-fpm.d/www.conf
sed -i 's/pm = dynamic/pm = static/1' /etc/php81/php-fpm.d/www.conf
sed -i "s/pm.max_children = 5/pm.max_children = $(nproc)/1" /etc/php81/php-fpm.d/www.conf
sed -i 's/;pm.max_requests = 500/pm.max_requests = 1000/1' /etc/php81/php-fpm.d/www.conf
sed -i 's/expose_php = On/expose_php = Off/1' /etc/php81/php.ini
sed -i 's/memory_limit = 128M/memory_limit = 256M/1' /etc/php81/php.ini
sed -i 's/max_execution_time = 30/max_execution_time = 60/1' /etc/php81/php.ini
...
curl -sLO https://getcomposer.org/download/latest-stable/composer.phar
